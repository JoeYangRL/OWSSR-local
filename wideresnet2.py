...
class WideResNet_Open(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=2, drop_rate=0.0):
        ...
        self.block1 = NetworkBlock(
            n, self.channels[0], self.channels[1], block, 1, drop_rate, activate_before_residual=True)
        # 2nd block
        self.block2 = NetworkBlock(
            n, self.channels[1], self.channels[2], block, 2, drop_rate)
        # 3rd block
        self.block3 = NetworkBlock(
            n, self.channels[2], self.channels[3], block, 2, drop_rate)
        # global average pooling and classifier
        self.bn1 = nn.BatchNorm2d(self.channels[3], momentum=0.001)
        self.relu = nn.LeakyReLU(negative_slope=0.1, inplace=True)
        self.simclr_layer = nn.Sequential(
                nn.Linear(self.channels[3], 128),
                nn.ReLU(),
                nn.Linear(128, 128),
        )
        #self.fc = CosineLinear(self.channels[3], num_classes)
        self.fc = nn.Linear(self.channels[3], num_classes)
        self.channels_last = self.channels[3]
    ...
    def change_output_dim(self, num_cls_new):
        ...
        new_fc = CosineLinear(self.channels_last, num_cls_new)
        new_fc.weight.data[:self.num_classes] = self.fc.weight.data
        self.fc = new_fc
        self.num_classes = num_cls_new


    def forward(self, x, open=True, feature=False, feat_only=False):
        #self.weight_norm()
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        ...
        out = out.view(-1, self.channels_last)
