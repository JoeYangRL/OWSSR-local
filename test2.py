...
def test(args, test_loader, model, val=False):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            pred_score, pred_idx = outputs.data.max(1)
            ...
            ind_unk = pred_score < 0.5
            pred_idx[ind_unk] = int(outputs.size(1))
    ...
    if not val:
        ...
        logger.info("Closed acc: {:.3f}".format(top1.avg)) #Closed acc: 所有known classes数据的top1 acc, 不看output_open的输出(相当于一个close set问题，排除了inlier检测成outlier的可能)
